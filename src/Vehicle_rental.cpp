...
void createBackup(const string& filename) {
    ifstream sourceFile("bin/" + filename, ios::binary);
    if (!sourceFile.is_open()) {
        cerr << "Error al abrir el archivo para respaldo: " << filename << endl;
        return;
    }

    string backupDir = "backup/";
    string backupFilename = backupDir + getCurrentDate() + "_" + filename;

    ofstream backupFile(backupFilename, ios::binary);
    if (!backupFile.is_open()) {
        cerr << "Error al crear el archivo de respaldo: " << backupFilename << endl;
        return;
    }

    backupFile << sourceFile.rdbuf();
    cout << "Respaldo creado: " << backupFilename << endl;
}
